...
>
> **v2.1 ใหม่:** (1) **ROOM login** — เลิก email login, ใช้ `roomUser` (เช่น room1) + `roomPassword`; field `ownerEmail`→`roomUser` ทุก collection; **admin login ใหม่ = `admin` / (ADMIN_PASS เดิม)** (env `ADMIN_USER`); ทุก device หลุด login (cookie เก่าใช้ไม่ได้). (2) **LINE player identity** — หน้า Home การ์ด "ผู้เล่น (LINE)" สแกน QR (jsqr) / กรอกเอง → เก็บ lineUserId(key)+lineEmail(personal) ลง session. (3) **External history API** `/api/ext/{history,stats,clips/:id,audio/:id}` query ด้วย lineUserId/email + header **`x-api-key: <HISTORY_API_KEY>`** (Secret Manager); ไม่โชว์ cost; วิดีโอ = proxy URL ที่ต้องใช้ key; ข้อมูล 3 วัน + leaderboard ถาวร (ไม่มี backfill).
