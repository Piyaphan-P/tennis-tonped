...
## Recent UI additions (v2.5, 2026-07-28)

- **History detail — session STATS card + share (like the post-session Summary):** at the TOP of the detail view (above the end-of-session summary card), a 4-tile widget — นาทีที่ตี (minutes) · ตีโดนลูก (shots) · ≈ความเร็วสวิงเฉลี่ย (avg km/h, shows "—" when unknown) · ≈เผาผลาญ (kcal) — plus topspin/backspin/flat % bars (hidden when spin can't be derived). A **Save/Share** button (two-step, same card look as the live Summary) exports the 1080×1920 stats card. Stats are persisted at session-end so they show for any past session (older sessions degrade gracefully: kcal recomputed from duration, speed "—", spin hidden).
- **History detail — Development Plan share button:** the "แนวทางพัฒนา" block now ends with a Save/Share button that exports the development-plan card (same one as the DevPlan screen). Shown only when the session has ranked faults.
- **External API — `stats` field:** `GET /api/ext/history` and `/api/ext/devplan` each session now includes a guaranteed-shape `stats: { durationMs, avgSpeedKmh, kcal, spin }` (nulls when a pre-v2.5 session lacks them) alongside the raw `summary`.
